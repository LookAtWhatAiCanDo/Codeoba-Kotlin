...
### Implementations:
All desktop source implementations inherit from the abstract base class [DesktopSourceAdapter](../core/src/desktopMain/kotlin/com/whataicando/codeoba/core/source/DesktopSourceAdapter.kt) under `desktopMain`, which centralizes common caching, folder validation (`getBaseDir()`), directory deletion, and command execution availability checking.
